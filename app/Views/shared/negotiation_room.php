...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
            <a href="<?= url($role === 'seller' ? '/seller/dashboard' : '/user/dashboard') ?>" class="text-muted text-decoration-none small">
                <i class="fas fa-arrow-left me-1"></i> Quay lại
            </a>
            <h4 class="mb-0 fw-bold mt-2">🏛️ <?= e($room['title']) ?> <span class="badge bg-light text-dark ms-2">#<?= $room['id'] ?></span></h4>
            <div class="small text-muted">Vai trò của bạn: <strong><?= $role === 'seller' ? 'Seller' : ($role === 'buyer' ? 'Khách hàng' : 'Admin') ?></strong></div>
        </div>
        <span class="badge fs-6 <?= $room['status'] === 'open' ? 'bg-primary' : ($room['status'] === 'resolved' ? 'bg-success' : 'bg-secondary') ?>">
            <?= $room['status'] === 'open' ? '🔓 Đang mở' : ($room['status'] === 'resolved' ? '✅ Đã giải quyết' : '🔒 Đã đóng') ?>
        </span>
    </div>

    <?php if (!empty($room['topic'])): ?>
    <div class="alert alert-warning border-0 shadow-sm">
        <strong>📋 Vấn đề cần giải quyết:</strong>
        <div class="mt-1"><?= nl2br(e($room['topic'])) ?></div>
    </div>
    <?php endif; ?>
    ...
    <div class="card border-0 shadow-sm" style="height: 70vh; display: flex; flex-direction: column;">
        <div class="card-header bg-white border-0 pt-3 d-flex align-items-center gap-2">
            <span class="badge bg-light text-dark"><i class="fas fa-user-shield me-1 text-primary"></i><?= e($room['admin_name']) ?></span>
            <span class="badge bg-light text-dark"><i class="fas fa-user me-1 text-info"></i><?= e($room['buyer_name']) ?></span>
            <span class="badge bg-light text-dark"><i class="fas fa-store me-1 text-success"></i><?= e($room['seller_name']) ?></span>
        </div>
        ...
        <div class="border-top p-3 bg-white">
            <form id="negSendForm" enctype="multipart/form-data">
                ...
                <div class="d-flex gap-2 align-items-end">
                    ...
                    <button type="button" class="btn btn-light" onclick="document.getElementById('negAttach').click()" title="Đính kèm ảnh">
                        <i class="fas fa-paperclip"></i>
                    </button>
                    <textarea name="message" id="negMessage" class="form-control" rows="1" placeholder="Nhập tin nhắn của bạn..." style="resize:none;"></textarea>
        ...
        <?php else: ?>
        <div class="border-top p-3 text-center text-muted small bg-light">🔒 Phòng đã đóng. Liên hệ admin để mở lại.</div>
        <?php endif; ?>
    </div>
</div>
...
(function() {
    ...
    function render(messages, currentUserId) {
        if (!messages.length) { box.innerHTML = '<div class="text-center text-muted py-5">Chưa có tin nhắn.</div>'; return; }
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (d.success) {
                    ...
                    alert(d.message || 'Không gửi được');
                }
