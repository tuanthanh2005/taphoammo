                        ...
                        <?php if (isset($sponsoredProducts[$i])): $product = $sponsoredProducts[$i]; ?>
                            ...
                            <div class="mb-3 position-relative <?= $i < 2 ? 'border-bottom pb-3' : '' ?>">
                                <div class="d-flex align-items-center mb-2">
                                    ...
                                    <div class="ms-2">
                                        <a href="<?= url('/product/' . $product['slug']) ?>" class="text-dark text-decoration-none fw-medium small hover-primary d-block stretched-link" title="<?= e($product['name']) ?>">
                                            <?= e($product['name']) ?>
                                        </a>
        ...
        <div class="col-12">
            ...
            <div class="card border-0 shadow-sm mb-4" style="border-radius: 10px; overflow: hidden;">
                <div class="card-body p-1">
                    <form action="" method="GET" class="d-flex flex-column flex-md-row w-100 align-items-stretch m-0">
                        <?php foreach($selectedCategories as $catId): ?>
                            <input type="hidden" name="category[]" value="<?= $catId ?>">
                        <?php endforeach; ?>
                        <input type="text" name="q" class="form-control border-0 bg-transparent flex-grow-1 px-3 px-md-4 py-2 border-bottom border-md-0" value="<?= e($_GET['q'] ?? '') ?>" placeholder="Tìm kiếm sản phẩm..." style="box-shadow: none; font-size: 0.95rem;">
